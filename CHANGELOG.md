...
### Added (2026-05-27 — PBS regression: selection predicted by breadth + connectivity)
- `analysis/phase2_pbs_regression.py` — selection-side mirror of the LOEUF joint regression; two primary outcomes (pbs1_african, pbs3_melanesian) plus European and South Asian PBS computed on the fly from FST columns (Afr–Mel reference pair, since no Eur–SAS FST exists); same four bivariate z-scored specs + a 5th `τ + log1p(KEGG) + LOEUF` orthogonal-axes spec; VIF, 95% CI, and a log1p(PBS) skew sensitivity pass
- Outputs: `output/phase2_pbs_regression.txt` (coefficient table, all 4 outcomes), `output/figure_phase2_pbs_regression.{png,pdf}` (African+Melanesian: AV plots + forest), `output/figure_phase2_pbs_regression_eur_sas.{png,pdf}` (European+South Asian)
- Findings (n=123–127): **African** PBS is significantly predicted by tissue specificity — τ β=−0.026 (p=0.014), breadth β=+0.024 (p=0.019); broadly-expressed → higher African PBS; **survives LOEUF control** (τ p=0.022, LOEUF itself ns) = direct orthogonal-axes test. **Melanesian** directionally consistent but ns (p=0.08–0.23, likely n=17 underpowered). **European/South Asian** null for τ/breadth/KEGG/betweenness — a lineage-specificity control showing the African signal isn't a generic metric artifact. Network centrality (KEGG, betweenness) predicts PBS in no population. log1p sensitivity: all coefficients directionally consistent (one trivial sign-flip on a β≈0 European τ). PBS right-skewed (African +3.5, others +1.2–1.5); untransformed kept as primary
- Caveat noted: Eur/SAS PBS use Afr–Mel reference structure (not the S.Asian outgroup of PBS-1/3), so they are valid lineage-specific drift estimates but not perfectly parallel scans
